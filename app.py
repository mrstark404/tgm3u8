...
def extract_m3u8_fallback(url):
    """Fallback method to scrape m3u8 URLs from the page using requests."""
    try:
        headers = {
        ...
        # Look for m3u8 in script tags or source tags
        for tag in soup.find_all(['source', 'script']):
            if tag.get('src') and '.m3u8' in tag.get('src'):
                return tag.get('src')
            if tag.string and '.m3u8' in tag.string:
                # Extract from script content
                import re
                urls = re.findall(r'https?://[^\s]*\.m3u8', tag.string)
                if urls:
                    return urls[0]
        logger.warning("No m3u8 found in fallback scraping.")
        return None
...
def extract_m3u8():
    ...
    cookies_file = request.args.get('cookies', None)  # Optional cookies file path
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            if not hls_url and 'url' in info and '.m3u8' in info['url']:
                ...
                logger.warning("Blob URL detected, attempting full extraction.")
            ...
            if "Unsupported URL" in str(e):
                ...
                hls_url = extract_m3u8_fallback(video_url)

    if not hls_url:
        return f"No m3u8 link found for {video_url}.", 404
    ...
    if download:
        ...
        try:
            ...
            ffmpeg.run(stream)
            filename = os.path.basename(urlparse(video_url).path) + '.mp4' or 'video.mp4'
